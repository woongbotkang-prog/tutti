...
export default function GigsPage() {
  ...
  useEffect(() => {
    const fetchGigs = async () => {
      ...
      let query = supabase
        .from('gigs')
        .select(`
          id, gig_type, title, is_paid, event_date, min_skill_level,
          region:regions(name),
      ...
      if (activeTab !== 'all') {
        query = query.eq('gig_type', activeTab)
      }

      const { data, error } = await query
      if (!error && data) {
        setGigs(data as unknown as GigItem[])
      }
      setLoading(false)
    }
    fetchGigs()
  ...
  const filtered = gigs.filter(g => {
    if (selectedInstrument !== '전체') {
      const hasInstrument = g.instruments?.some(i => i.instrument?.name === selectedInstrument)
    ...
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <main className="max-w-lg mx-auto px-4 py-3 space-y-3">
        ...
        {!loading && filtered.map(gig => {
          const instrumentNames = gig.instruments
            ...
            <Link key={gig.id} href={`/gigs/${gig.id}`}>
              <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm hover:shadow-md hover:border-indigo-200 transition-all">
                <div className="flex items-start justify-between mb-2">
                  ...
                  <span className="text-xs text-gray-400">{gig.region?.name || '지역미정'}</span>
                </div>
